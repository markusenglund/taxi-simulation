...
public class FareGraph : MonoBehaviour
{
    ...
    string headingText = "Average fare";
    ...
    public static FareGraph Create(City city1, City city2)
    {
        Transform canvas = GameObject.Find("Canvas").transform;
        Transform graphPrefab = Resources.Load<Transform>("Graphs/FareGraph");
        Transform graphTransform = Instantiate(graphPrefab, canvas);

        FareGraph graph = graphTransform.GetComponent<FareGraph>();
    ...
    private float GetAverageFare(List<Trip> trips)
    {
        if (trips.Count == 0)
        {
            return 0;
        }
        return trips.Average(trip => trip.tripCreatedData.fare.total);
    }
    ...
    private IEnumerator UpdateCurves()
    {
        ...
        while (simulationTime < staticCity.simulationSettings.simulationLengthHours)
        {
            ...
            float averageStaticFare = GetAverageFare(staticTrips);
            float averageSurgeFare = GetAverageFare(surgeTrips);

            lastStaticValues.Enqueue((averageStaticFare, simulationTime));
            ...
            lastSurgeValues.Enqueue((averageSurgeFare, simulationTime));
            ...
            float smoothedAverageStaticFare = lastStaticValues.Average(x => x.value);
            float smoothedAverageSurgeFare = lastSurgeValues.Average(x => x.value);
            ...
            Vector3 staticPosition = ConvertValueToGraphPosition(new Vector3(graphTime, smoothedAverageStaticFare, 0));
            ...
            Vector3 surgePosition = ConvertValueToGraphPosition(new Vector3(graphTime, smoothedAverageSurgeFare, 0));
