...
class ProsAndConsList(Scene):
    def construct(self):
        ...
        pros1 = Text("Lower waiting times\nduring high demand").next_to(plus1, RIGHT)
        plus2 = MathTex("+").scale(2).next_to(plus1, DOWN * 2).set_color("#00FF00")
        pros2 = Text("Lower fares during\nlow demand").next_to(plus2, RIGHT)
        plus3 = MathTex("+").scale(2).next_to(plus2, DOWN * 2.4).set_color("#00FF00")
        pros3 = Text("More rides allocated\nto people most in need").next_to(plus3, RIGHT)
        ...
        con1 = Text("Higher fares\nduring high demand").next_to(minus1, RIGHT)
        minus2 = MathTex("-").scale(2).next_to(minus1, DOWN * 4).set_color("#FF0000")
        con2 = Text("Fewer rides overall").next_to(minus2, RIGHT)
