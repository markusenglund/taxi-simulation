...
class TotalCostCalculation(Scene):
    def construct(self):
        ...
        equationStart = Tex(r"Total cost", r" = Fare", r" + Time to destination",  r" × Value of time", tex_template=TexFontTemplates.droid_sans ,font_size=DEFAULT_FONT_SIZE)
        # equationWithValues = MathTex(r"= \$12.00", r"+ 0.35 \, hrs", r"× \$141.20/hr", font_size=DEFAULT_FONT_SIZE)
        equationWithValues = Tex(r"= \$12.00", r" + 24 min", r" × \$68.00/hr", tex_template=TexFontTemplates.droid_sans, font_size=DEFAULT_FONT_SIZE)
        equationWithValues.next_to(equationStart, DOWN).align_to(equationStart[1], LEFT)
        equationWithValues2 = Tex(r"= \$12.00", r" + \$27.20", tex_template=TexFontTemplates.droid_sans, font_size=DEFAULT_FONT_SIZE)
        equationWithValues2.next_to(equationStart, DOWN).align_to(equationStart[1], LEFT)
        equationResult = Tex(r"= \$39.20", tex_template=TexFontTemplates.droid_sans, font_size=DEFAULT_FONT_SIZE)
